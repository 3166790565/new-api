...
export function RegistrationCodesPrimaryButtons() {
  ...
  const { setOpen, triggerRefresh } = useRegistrationCodes()
  ...
  return (
    <>
      ...
      <ConfirmDialog
        ...
        className='max-w-md'
        title={t('Delete Invalid Registration Codes?')}
        desc={
          <>
            {t('This will delete all')} <strong>{t('disabled')}</strong>
            {t(',')} <strong>{t('expired')}</strong>
            {t(', and')} <strong>{t('used up')}</strong> {t('registration codes.')}
            <br />
