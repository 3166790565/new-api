...
func InitOptionMap() {
	...
	common.OptionMap["LogUserRealModelEnabled"] = strconv.FormatBool(common.LogUserRealModelEnabled)
...
func validateOptionValue(key string, value string) error {
	...
	if err := operation_setting.ValidateContributionOption(key, value); err != nil {
		return err
	}
...
func updateOptionMap(key string, value string) (err error) {
	...
	if strings.HasSuffix(key, "Enabled") || key == "DefaultCollapseSidebar" || key == "DefaultUseAutoGroup" || key == "SMTPForceAuthLogin" || key == "SMTPInsecureSkipVerify" {
		...
		case "LogUserRealModelEnabled":
			common.LogUserRealModelEnabled = boolValue
